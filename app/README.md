...
헤더의 톱니(`자세 노드 주소`) 버튼을 누르면 숫자 키패드가 뜬다. Pi 4 의 IP 를 찍고
`저장` 하면 즉시 그 주소를 보기 시작하고, 다음에 켤 때도 그 주소를 쓴다. `데모로` 를
누르면 저장된 주소를 지우고 화면 내장 데모로 돌아간다.

**키보드를 쓰지 않는다.** 보드에 USB 키보드가 꽂혀 있으리라 가정할 수 없고
Flutter eLinux 에는 화면 키보드가 없어서, `TextField` 대신 키패드를 직접 그렸다.
그래서 호스트 이름은 못 치고 IPv4 만 받는다. 포트는 `8765` 고정이다.

주소 우선순위는 **저장된 값 → 빌드에 박은 `DESKMATE_HUB_URL` → 데모** 순이다.
`--dart-define` 은 이제 "공장 기본값" 이지 유일한 경로가 아니다.

적히는 곳은 순서대로 아래와 같고, 쓸 수 있는 첫 곳을 쓴다. 다 실패해도 앱은 돌고
화면이 "이번 실행에만 적용됩니다" 라고 말해 준다.

| 순서 | 경로 |
|---|---|
| 1 | `$DESKMATE_CONFIG_DIR/hub.txt` (배포 때 못 박고 싶을 때) |
| 2 | `$XDG_CONFIG_HOME/camtest/hub.txt` |
| 3 | `$HOME/.config/camtest/hub.txt` |
| 4 | `/tmp/camtest/hub.txt` (재부팅하면 사라진다) |

이 화면을 만든 이유는 보드가 개발 노트북과 다른 망에 있기 때문이다. 주소를 빌드에만
박아 두면 Pi 4 의 DHCP 주소가 바뀔 때마다 다시 구워서, 보드와 같은 망에 있는 사람에게
설치를 다시 부탁해야 한다.
